...
function CoachDashboard({ onSignOut, userId }) {
  ...
  async function handleCreate() {
    if (programmeName === '' || studentName === '') { alert('Please fill in the programme name and student email!'); return }
    const { data, error } = await supabase.from('programmes').insert([{ coach_id: userId, student_email: studentName, name: programmeName, due_date: dueDate, activities }]).select()
    if (error) { alert('Error saving programme: ' + error.message); return }
    setProgrammes([data[0], ...programmes])
    setProgrammeName(''); setStudentName(''); setDueDate(''); setActivities([])
  ...
  function addActivity() {
    if (newActivityName.trim() === '') { alert('Please enter an activity name!'); return }
    let detail = ''
    ...
    setActivities([...activities, { name: newActivityName.trim(), detail, requiresVideo: newActivityVideo }])
    setNewActivityName(''); setNewActivitySets('3'); setNewActivityReps('10')
    setNewActivityDuration(''); setNewActivityRepType('reps'); setNewActivityVideo(false)
  }

  if (showProfile) return <CoachProfile userId={userId} onBack={() => setShowProfile(false)} />
  if (selectedProgramme) return <ProgrammeDetail programme={selectedProgramme} onBack={() => setSelectedProgramme(null)} />
  if (showStudents) return <StudentsPage onBack={() => setShowStudents(false)} userId={userId} />
  if (showSubmissions) return <SubmissionsPage onBack={() => setShowSubmissions(false)} userId={userId} />
  if (showMessages) return <MessagesPage userId={userId} role="coach" onBack={() => setShowMessages(false)} />
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#F7F7F5' }}>
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '32px 24px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          ...
          {!showForm && (
            <button onClick={() => setShowForm(true)} style={{ padding: '9px 18px', fontSize: '13px', fontWeight: '600', cursor: 'pointer', backgroundColor: '#1D9E75', color: 'white', border: 'none', borderRadius: '8px' }}>+ New programme</button>
          )}
        </div>
        ...
        {programmes.map((programme, index) => (
          <div key={programme.id || index} style={{ backgroundColor: 'white', borderRadius: '14px', padding: '20px 24px', marginBottom: '12px', border: '1px solid #eee', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div>
              <div style={{ fontWeight: '600', fontSize: '15px' }}>{programme.name}</div>
              <div style={{ fontSize: '13px', color: '#888', marginTop: '4px' }}>👤 {programme.student_email} · 📅 Due {programme.due_date || 'No date set'} · {programme.activities?.length || 0} activities</div>
            </div>
            <button onClick={() => setSelectedProgramme(programme)} style={{ padding: '7px 16px', fontSize: '13px', cursor: 'pointer', borderRadius: '8px', border: '1px solid #eee', backgroundColor: 'white', color: '#555', fontWeight: '500' }}>View →</button>
          </div>
        ))}

        {showForm && (
          <div style={{ backgroundColor: 'white', borderRadius: '16px', padding: '28px', border: '1px solid #eee', marginTop: '16px' }}>
            ...
            <div style={{ marginBottom: '16px' }}>
              ...
              <input type="text" placeholder="e.g. Upper body strength week 2" value={programmeName} onChange={(e) => setProgrammeName(e.target.value)} style={{ width: '100%', padding: '10px 14px', border: '1px solid #eee', borderRadius: '10px', fontSize: '14px', outline: 'none' }} />
            </div>

            <div style={{ marginBottom: '16px' }}>
              <label style={{ display: 'block', fontSize: '13px', fontWeight: '500', color: '#666', marginBottom: '6px' }}>Student email</label>
              <input type="text" placeholder="Student's email e.g. student@gmail.com" value={studentName} onChange={(e) => setStudentName(e.target.value)} style={{ width: '100%', padding: '10px 14px', border: '1px solid #eee', borderRadius: '10px', fontSize: '14px', outline: 'none' }} />
            </div>

            <div style={{ marginBottom: '16px' }}>
              <label style={{ display: 'block', fontSize: '13px', fontWeight: '500', color: '#666', marginBottom: '6px' }}>Due date</label>
              <input type="date" value={dueDate} onChange={(e) => setDueDate(e.target.value)} style={{ width: '100%', padding: '10px 14px', border: '1px solid #eee', borderRadius: '10px', fontSize: '14px', outline: 'none' }} />
            </div>
            ...
            <div style={{ marginBottom: '24px' }}>
              ...
              {activities.map((act, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 14px', backgroundColor: '#f9f9f9', borderRadius: '8px', marginBottom: '8px', fontSize: '14px' }}>
                  <div style={{ flex: 1 }}>
                  ...
                  <button onClick={() => setActivities(activities.filter((_, idx) => idx !== i))} style={{ padding: '3px 10px', fontSize: '12px', cursor: 'pointer', borderRadius: '6px', border: '1px solid #ffcccc', backgroundColor: '#fff5f5', color: '#cc0000' }}>Remove</button>
                </div>
              ))}

              <div style={{ padding: '16px', border: '1px dashed #ddd', borderRadius: '10px', marginTop: '8px', backgroundColor: '#fafafa' }}>
                <div style={{ marginBottom: '10px' }}>
                  <label style={{ fontSize: '12px', color: '#888', display: 'block', marginBottom: '4px' }}>Activity name</label>
                  <input type="text" placeholder="e.g. Push-ups, Squats, Plank hold..." value={newActivityName} onChange={(e) => setNewActivityName(e.target.value)} style={{ width: '100%', padding: '8px 12px', border: '1px solid #eee', borderRadius: '8px', fontSize: '13px', outline: 'none', backgroundColor: 'white' }} />
                </div>

                <div style={{ display: 'flex', gap: '10px', marginBottom: '10px' }}>
                  <div style={{ flex: 1 }}>
                    <label style={{ fontSize: '12px', color: '#888', display: 'block', marginBottom: '4px' }}>Sets</label>
                    <select value={newActivitySets} onChange={(e) => setNewActivitySets(e.target.value)} style={{ width: '100%', padding: '8px 12px', border: '1px solid #eee', borderRadius: '8px', fontSize: '13px', outline: 'none', backgroundColor: 'white' }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <select value={newActivityRepType} onChange={(e) => setNewActivityRepType(e.target.value)} style={{ width: '100%', padding: '8px 12px', border: '1px solid #eee', borderRadius: '8px', fontSize: '13px', outline: 'none', backgroundColor: 'white' }}>
                  ...
                  {newActivityRepType === 'reps' && (
                    <div style={{ flex: 1 }}>
                      ...
                      <select value={newActivityReps} onChange={(e) => setNewActivityReps(e.target.value)} style={{ width: '100%', padding: '8px 12px', border: '1px solid #eee', borderRadius: '8px', fontSize: '13px', outline: 'none', backgroundColor: 'white' }}>
                  ...
                  {newActivityRepType === 'time' && (
                    <div style={{ flex: 1 }}>
                      ...
                      <select value={newActivityDuration} onChange={(e) => setNewActivityDuration(e.target.value)} style={{ width: '100%', padding: '8px 12px', border: '1px solid #eee', borderRadius: '8px', fontSize: '13px', outline: 'none', backgroundColor: 'white' }}>
                  ...
                  {newActivityRepType === 'amrap' && (
                    <div style={{ flex: 1, display: 'flex', alignItems: 'flex-end', paddingBottom: '1px' }}>
                      <div style={{ width: '100%', padding: '8px 12px', backgroundColor: '#E1F5EE', borderRadius: '8px', fontSize: '13px', color: '#0F6E56', fontWeight: '500', textAlign: 'center' }}>As many reps as possible</div>
                    </div>
                  )}
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  <button onClick={addActivity} style={{ padding: '8px 16px', backgroundColor: '#1D9E75', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '13px', fontWeight: '500' }}>+ Add activity</button>
                </div>
              </div>
            </div>

            <div style={{ display: 'flex', gap: '10px' }}>
              <button onClick={handleCreate} style={{ padding: '10px 24px', backgroundColor: '#1D9E75', color: 'white', border: 'none', borderRadius: '10px', cursor: 'pointer', fontSize: '14px', fontWeight: '600' }}>Save programme</button>
              <button onClick={() => setShowForm(false)} style={{ padding: '10px 24px', cursor: 'pointer', borderRadius: '10px', border: '1px solid #eee', fontSize: '14px', fontWeight: '500', backgroundColor: 'white' }}>Cancel</button>
            </div>
          </div>
        )}
      </div>
