...
function DiscoverTab({ userId, role }) {
  ...
  const filtered = coaches.filter(c => {
    ...
    const matchSpec = !filterSpeciality || (c.specialities || '').toLowerCase().includes(filterSpeciality.toLowerCase())
    ...
    return (
      <div style={{ backgroundColor: '#0a0a0a', minHeight: '100vh', color: 'white' }}>
        <div style={{ padding: '56px 20px 20px 20px', display: 'flex', alignItems: 'center', gap: '12px' }}>
          <button onClick={() => setSelectedCoach(null)} style={{ background: '#111', border: '1px solid #222', borderRadius: '10px', color: 'white', padding: '8px 14px', cursor: 'pointer', fontSize: '13px' }}>← Back</button>
        ...
        <div style={{ padding: '0 20px' }}>
          <div style={{ backgroundColor: '#111', borderRadius: '20px', padding: '28px', border: '1px solid #1a1a1a', marginBottom: '16px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '20px' }}>
              ...
              <div>
                ...
                <div style={{ fontSize: '13px', color: '#666', marginTop: '4px' }}>
                  {selectedCoach.years_experience > 0 && `${selectedCoach.years_experience} yrs exp`}
            ...
            {selectedCoach.bio && (
              <p style={{ fontSize: '14px', color: '#aaa', lineHeight: '1.7', marginBottom: '20px' }}>{selectedCoach.bio}</p>
            )}
  ...
  return (
    <div style={{ backgroundColor: '#0a0a0a', minHeight: '100vh', color: 'white' }}>
      ...
      <div style={{ padding: '56px 20px 20px 20px' }}>
        ...
        <div style={{ fontSize: '14px', color: '#555' }}>Find your perfect coach</div>
      </div>
          ...
          <div style={{ display: 'flex', gap: '8px', paddingBottom: '4px' }}>
            <button
              ...
              style={{
                padding: '7px 14px',
                borderRadius: '20px',
                border: `1px solid ${!filterSpeciality ? '#1D9E75' : '#222'}`,
                backgroundColor: !filterSpeciality ? 'rgba(29,158,117,0.15)' : 'transparent',
                color: !filterSpeciality ? '#1D9E75' : '#555',
                fontSize: '12px',
                fontWeight: '600',
                cursor: 'pointer',
                whiteSpace: 'nowrap'
            ...
            {allSpecialities.map((s, i) => (
              <button
                ...
                style={{
                  padding: '7px 14px',
                  borderRadius: '20px',
                  border: `1px solid ${filterSpeciality === s ? '#1D9E75' : '#222'}`,
                  backgroundColor: filterSpeciality === s ? 'rgba(29,158,117,0.15)' : 'transparent',
                  color: filterSpeciality === s ? '#1D9E75' : '#555',
                  fontSize: '12px',
                  fontWeight: '600',
                  cursor: 'pointer',
                  whiteSpace: 'nowrap'
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '14px', marginBottom: '14px' }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: '12px', color: '#555' }}>
                    {coach.years_experience > 0 && `${coach.years_experience} yrs · `}
