...
function ProfileTab({ userId, role, onSignOut }) {
  ...
  if (showStudents) {
    return <StudentsPage userId={userId} onBack={() => setShowStudents(false)} />
  }
      ...
      {editing ? (
        ...
        <div style={{ padding: '0 20px 32px 20px' }}>
          ...
          <div style={{ backgroundColor: '#111', borderRadius: '20px', padding: '24px', border: '1px solid #1a1a1a', marginBottom: '12px', textAlign: 'center' }}>
            <div style={{ width: '80px', height: '80px', borderRadius: '50%', background: 'linear-gradient(135deg, #1D9E75, #0a5c43)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '32px', fontWeight: '800', color: 'white', margin: '0 auto 16px auto', border: '3px solid #1D9E75' }}>
          ...
          {/* Students button — coaches only */}
          {role === 'coach' && (
            <button
              onClick={() => setShowStudents(true)}
              style={{ width: '100%', padding: '16px 20px', backgroundColor: '#111', border: '1px solid #1a1a1a', borderRadius: '16px', color: 'white', fontSize: '14px', fontWeight: '600', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                <span style={{ fontSize: '20px' }}>👥</span>
                <div style={{ textAlign: 'left' }}>
                  <div style={{ fontSize: '14px', fontWeight: '600' }}>My Students</div>
                  <div style={{ fontSize: '12px', color: '#555', marginTop: '2px' }}>Manage invites and student list</div>
                </div>
              </div>
