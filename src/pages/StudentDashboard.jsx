...
function StudentDashboard({ onSignOut, userId }) {
  ...
  async function acceptInvite(invite) {
    await supabase
      .from('coach_students')
      .update({ status: 'accepted', student_id: userId })
      .eq('id', invite.id)
    loadPendingInvites()
  }
              ...
              <div key={programme.id} style={{ backgroundColor: 'white', borderRadius: '16px', padding: '24px', marginBottom: '16px', border: '1px solid #eee' }}>
                ...
                {!isSubmitted ? (
                  <div>
                    ...
                    <button onClick={() => handleSubmit(programme.id)} style={{ padding: '11px 28px', backgroundColor: '#1D9E75', color: 'white', border: 'none', borderRadius: '10px', cursor: 'pointer', fontSize: '14px', fontWeight: '600' }}>Submit to coach →</button>
                  </div>
                ) : (
                  <div style={{ padding: '16px 20px', backgroundColor: '#E1F5EE', border: '1px solid #1D9E75', borderRadius: '12px' }}>
